...
## Plot a graph
```{r, echo=F, fig.height=3, include=F, warning=F}
p1<-ggplot(
  data = finalw,
...
## Run regressions
```{r}
regdest<-lm(dep_delay~dest,destairport)
regdestCancel<-glm(canceled~dep_delay+dest, desairport, family = binomial(logit))
```
...
```{r}
regplane<-lm(dep_delay~year.x+factor(model),airplains)
regplaneCancel<-glm(canceled~dep_delay+year.x+modle,airplains,family = binomial(logit))
```
